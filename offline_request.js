...
fs.readFile('data/large_messageData.txt', function(err, data) {
	// TODO check if it's a group (who are the participants in the group)
	const messageList = JSON.parse(data);
	//console.log(Math.max.apply(null, helper.flattenArrayByKey(messageList, "messageCount")));
	...
	//console.log(sortedMessageList[0].messageCount);
	...
	function findOldBestFriends() { // Returns old best friends
		...
		while(bestFriends.length < 3 && i < sortedMessageList.length) {
			...
			if(sortedMessageList[i].lastMessageTimestamp < dateOneYearAgo && !friend.isGroup) bestFriends.push(sortedMessageList[i]);  
			i++;
		}
	...
	function findRecentBestFriends() { // Returns new best friends
		...
		while(bestFriends.length < 3 && i < sortedMessageList.length) {
			var friend = sortedMessageList[i];
			if(friend.lastMessageTimestamp >= dateOneYearAgo && !friend.isGroup) bestFriends.push(friend); 
			i++; 
		}
		return[bestFriends[0], bestFriends[1], bestFriends[2]];
	}
	...
	//console.log(messageList.find(thread => thread.isGroup));
   	console.log(messageList[0]);
});
/*
